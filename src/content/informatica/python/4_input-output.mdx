---
title: "Input e Output"
order: 4
draft: false
lastUpdateDate: "2026-01-12"
subject: "Python"
category: "Informatica"
difficulty: "facile"
tags: ["python", "programmazione", "introduzione", "input", "output"]
author: "Dennis Turco"
videoLesson: false
---

## 1 Output

L'**output** permette di stampare a schermo.

La funzione che si usa per stampare (output) a schermo qualcosa è la funzione `print()`

La formattazione dell'output può essere fatta in più modi:

1. **Using formatted string literals**
2. **Using format()**
3. **Using % Operator**

<Aside variant="info">
nessun modo è migliore di un altro, la scelta è puramente personale.
</Aside>

### 1.1 Using formatted string literals

Si inserisce il carattere `f` o `F` prima di `'` e si mettono le `{}` con il nome della variabile dove la si vuole stampare.

```py
x = 5
y = 10

print(f'x is: {x}; y is: {y}')

# Output:
#    x is: 5; y is: 10
```

### 1.2 Using format()

Simile a prima, con la differenza che non si mette il carattere f prima di ', né il nome delle variabili all'interno di {}.
Si aggiunge `.format()` dopo la chiusura di `'` con argomento il nome delle variabili nell'ordine in cui si vogliono stampare.

```py
x = 5
y = 10

print('x is: {}; y is: {}'.format(x, y))

# Output:
#    x is: 5; y is: 10
```

### 1.3 Using % Operator

Stile simile al printf del linguaggio C.
Si mette il simbolo % preceduto dalla lettera che fa riferimento al tipo della variabile che si vuole stampare.

- %d $\rightarrow$ integer
- %f $\rightarrow$ float
- %s $\rightarrow$ string
- %x $\rightarrow$ hexadecimal
- %o $\rightarrow$ octal

```py
x = 5
y = 10

print('x is: %d; y is: %d' % (x, y))

# Output:
#    x is: 5; y is: 10
```

## 2. Input

L'input permette di scrivere qualcosa a tastiera.

La funzione che si usa per scrivere (input) a tastiera è la funzione `input()`

- Ottenere l'input senza messaggio:

    ```py
    result = input()
    ```

- Ottenere l'input con messaggio:

    ```py
    result = input("Insert some text: ")
    ```

- Ottenere l'input intero (int) con messaggio:

    ```py
    result = int(input("Insert an integer: "))
    ```

<Aside variant="warning">
    La funzione `input()` restituisce sempre una **stringa**, quindi è necessario convertire il valore se serve un numero.
</Aside>

## 3. Variabili

Le **variabili** servono per memorizzare dati che possono essere usati nel programma.

Si assegna un valore con l'operatore **=**:
```py
nome = "Anna"
eta = 20
altezza = 1.65
```

Possibile poi usare queste variabili, ad esempio per la stampa:
```py
print(f"{nome} ha {eta} anni ed è alta {altezza} metri.")
```

Le variabili (sono appunto variabili e quindi possono campbiare) possono cambiare valore nel tempo:
```py
x = 5
x = x + 3
print(x)  # Output: 8
```

## 4. Tipi di dati primitivi
I tipi di dati più comuni in Python sono:
| Tipo    | Nome                         | Esempio                  |
| ------- | ---------------------------- | ------------------------ |
| `int`   | Numero intero                | `x = 10`                 |
| `float` | Numero decimale              | `x = 3.14`               |
| `str`   | Stringa di testo             | `x = "Ciao"`             |
| `bool`  | Valore booleano (vero/falso) | `x = True` o `x = False` |

Possibile verificare il tipo con la funzione `type()`:
```py
x = 3.14
print(type(x))  # Output: <class 'float'>
```

## 5. Quiz a risposta multipla

import Quiz from '../../../components/Quiz.jsx';

<Quiz client:load questions={[
{
question: "1) Quale funzione si usa in Python per stampare un messaggio a schermo?",
answers: {
a: "input()",
b: "write()",
c: "print()",
d: "output()"
},
correctAnswer: "c"
},
{
question: "2) Qual è il tipo di dato restituito dalla funzione input()?",
answers: {
a: "int",
b: "float",
c: "bool",
d: "str"
},
correctAnswer: "d"
},
{
question: "3) Qual è il modo corretto per stampare una variabile usando le f-string?",
answers: {
a: "print('x is {x}')",
b: "print(f'x is: {x}')",
c: "print(format(x))",
d: "print(%x)"
},
correctAnswer: "b"
},
{
question: "4) Cosa fa questa istruzione? eta = int(input('Inserisci età: '))",
answers: {
a: "Legge una stringa",
b: "Legge un numero decimale",
c: "Legge un numero intero convertendo l'input",
d: "Stampa l'età"
},
correctAnswer: "c"
},
{
question: "5) Quale tra questi è un tipo di dato booleano valido in Python?",
answers: {
a: "'true'",
b: "1",
c: "True",
d: "False"
},
correctAnswer: "c"
}
]} />

## 6. Esercizi

### 6.1 Esercizio

Scrivi un programma che:
- Chieda all'utente il suo nome e la sua età
- Calcoli quanti anni avrà tra 5 anni
- Stampi una frase di riepilogo

Esempio Output:

Come ti chiami? Anna <br/>
Quanti anni hai? 20 <br/>
Ciao Anna, tra 5 anni avrai 25 anni! <br/>

